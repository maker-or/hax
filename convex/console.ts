...
function createClientSecretTraceId(): string {
...
function logCreateClientSecretTrace(
	traceId: string,
	step: string,
	payload: Record<string, unknown>,
): void {
	console.log(
		`[console][clientSecrets][trace:${traceId}] ${step}`,
...
function getClientSecretLastFour(secret: WorkOSClientSecret): string {
	if (typeof secret.secret === "string" && secret.secret.length >= 4) {
		return secret.secret.slice(-4);
	}
	if (
		typeof secret.secret_hint === "string" &&
...
function getWorkosRequestHeaders(options: RequestInit): Record<string, string> {
	const headers = {
		...workosHeaders(),
...
export const updateApp = action({
	...
	handler: async (ctx, args): Promise<void> => {
		...
		const hasConvexPatch =
			Object.keys(workosPayload).length > 0 || normalizedDomains !== undefined;
		if (!hasConvexPatch) {
			throw new ConvexError({
	...
	handler: async (
		...
		try {
			...
			logCreateClientSecretTrace(traceId, "workos_response_received", {
				...
				secretLength:
					typeof secret.secret === "string" ? secret.secret.length : undefined,
			...
			logCreateClientSecretTrace(traceId, "database_write_skipped", {
				reason:
					"Client secrets are not stored in Convex. WorkOS returns the plaintext secret once and the frontend keeps it in memory only.",
			});
