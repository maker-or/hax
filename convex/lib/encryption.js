...
async function encrypt(plaintext) {
    ...
    const cipherBase64 = btoa(String.fromCharCode(...Array.from(new Uint8Array(cipherBuffer))));
...
async function decrypt(encryptedValue) {
    ...
    const decryptedBuffer = await crypto.subtle.decrypt({ name: ALGORITHM, iv }, key, cipherBuffer);
