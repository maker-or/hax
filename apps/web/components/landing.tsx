  ...
  return (
    <>
      {text.split(" ").map((word, i) => (
        <span key={`${word}-${i}`} className={`inline-block ${className}`} style={{ opacity: 0, filter: 'blur(10px)' }}>
          {word}&nbsp;
        </span>
      ))}
    </>
...
const CommandBox = ({ command }: { command: string }) => {
  ...
  return (
    <div
      ...
      tabIndex={0}
    >
      <code className="text-[#d4b4e2] text-[15px] font-mono tracking-wide">{command}</code>
 ...
 */
const Step = ({ number, title, command, link }: { number: number; title: string; command?: string; link?: { text: string; href: string } }) => {
  return (
    <div className="mb-8">
      <div className="flex gap-4">
        <div className="text-[#d4b4e2] text-[18px] font-normal min-w-fit">{number}.</div>
        <div className="flex-1">
          <p className="text-[#7D6988] text-[18px] leading-relaxed">
            ...
            {link && (
              <>
                {' '}
                <a href={link.href} className="text-[#d4b4e2] hover:opacity-80 transition-opacity cursor-pointer underline">
                  {link.text}
                </a>
              </>
            )}
...
const USAGE_CONTENT_DEV = {
  ...
  steps: [
    ...
    {
      number: 4,
      title: "When a user is successfully authenticated, they will be redirected to your provided redirect URI along with the following information",
    },
  ],
  response: `{
...
const USAGE_CONTENT_USER = {
  ...
  steps: [
    ...
    {
      number: 4,
      title: "Now you can use your AI subscriptions in any tool support login with your Polaris account",
    },
  ],
};
...
export default function Home() {
  ...
  const content = currentPage === 'about'
    ? ABOUT_CONTENT
    : userType === 'dev'
      ? USAGE_CONTENT_DEV
      : USAGE_CONTENT_USER;
  ...
  const isDevContent = currentPage === 'usage' && userType === 'dev';
    ...
    >
      <div className="w-175 h-full border-x border-dashed border-[#403149] px-3 relative">
        ...
        <div className="w-full h-full border-x border-dashed border-[#403149] flex flex-col relative overflow-y-auto">
          ...
          <div className={`border-y border-dashed border-[#403149] px-10 py-6 ${currentPage === 'usage' ? 'mt-0' : 'mt-10'}`}>
          ...
          <div className="px-10 py-8 space-y-6 text-[20px] leading-relaxed font-normal pr-16">
            {currentPage === 'usage' && userType === 'dev' ? (
              <div className="space-y-8">
                <p>
                  <SplitWords text={USAGE_CONTENT_DEV.paragraphs[0]} className="para-word" />
                </p>
